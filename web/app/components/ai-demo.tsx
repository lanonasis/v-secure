...
function getAiChatUrl(): string {
  ...
  // 2. If SUPABASE_URL already includes the ai-chat edge function path, use it directly
...
export function AiDemo() {
  ...
  const sendToAI = async (conversationHistory: ChatMessage[]): Promise<string> => {
    if (!isConfigured) {
      throw new Error('AI Chat not configured. Set NEXT_PUBLIC_SUPABASE_URL in .env.local')
    }
