...
- **Connection String**: `DATABASE_URL` (exported in environment)
...
### oauth-client
```typescript
// Uses DATABASE_URL or Supabase connection
...
0 * * * * psql "$DATABASE_URL" -c "SELECT cleanup_expired_oauth_codes();"

# Daily: Clean expired tokens
0 0 * * * psql "$DATABASE_URL" -c "SELECT cleanup_expired_oauth_tokens();"

# Monthly: Archive old analytics
0 0 1 * * psql "$DATABASE_URL" -c "SELECT cleanup_old_analytics(12);"
```
