...
- **Note**: `DATABASE_URL` env var points to Neon, not this database!
...
2. **Auth-Gateway → Neon** (Direct Postgres)
   ...
   - Uses `DATABASE_URL` which points to Neon
...
- `apps/onasis-core/services/auth-gateway/db/client.ts`
  ...
  - `dbPool` - PostgreSQL pool (connects via DATABASE_URL to Neon)
...
```
DATABASE_URL=postgres://...@ep-snowy-surf-adqqsawd-pooler.c-2.us-east-1.aws.neon.tech/neondb
WEBHOOK_SECRET=d45932caace861750bed85b45ee56c391c967919d9a12df049d6df4aaa248bc1
NEON_DATABASE_URL=... (same as DATABASE_URL for sync functions)
```
...
2. Verify `DATABASE_URL` in edge function settings
